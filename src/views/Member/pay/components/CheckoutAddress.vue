...
// import { CityListType } from '@/components/city/index.vue'
import Message from "@/components/message";

const { checkoutStore: checkout } = useStore();
...
watch(visible, (value) => {
  if (value) {
    selectAddress.value = checkout.showUserAddress!;
  }
});
...
<template>
  <div class="checkout-address">
    <div class="text">
      <div v-if="!checkout.showUserAddress" class="none">
        您需要先添加收货地址才可提交订单。
      </div>
      <template v-else>
        <ul>
          <li>
            <span>收<i />货<i />人：</span
            >{{ checkout.showUserAddress.receiver }}
          </li>
          <li><span>联系方式：</span>{{ checkout.showUserAddress.contact }}</li>
          <li>
            <span>收货地址：</span
            >{{
              checkout.showUserAddress.fullLocation +
              " " +
              checkout.showUserAddress.address
            }}
          </li>
        </ul>
    ...
    <XtxDialog v-model="dialogVisible" title="添加收货地址">
      <template #body>
    ...
    <XtxDialog v-model="visible" title="切换收货地址">
      <template #body>
        <div
